...
MODEL_MAPPING = {
    'bert': BertClassifier,
    'bert-pu': BertPUClassifier,
    'roberta': RobertaClassifier,
    'roberta-pu': RobertaPUClassifier,
}
...
def train(args, data_processor, model, tokenizer):
    ...
    for _ in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            loss, logits = outputs[0], outputs[1]  # model outputs are always tuple in transformers (see doc)
...
def evaluate(args, data_processor, model, tokenizer, prefix=""):
    ...
    all_input_ids = None
    all_phrase_spans = None
    all_num_phrases = None
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            if label_truth is None:
                label_truth = inputs["labels"].detach().cpu().numpy()
                label_predicted = logits.detach().cpu().numpy()
                all_input_ids = inputs["input_ids"].detach().cpu().numpy()
                all_phrase_spans = inputs["phrase_spans"].detach().cpu().numpy()
                all_num_phrases = inputs["num_phrases"].detach().cpu().numpy()
            else:
                label_truth = np.append(label_truth, inputs["labels"].detach().cpu().numpy(), axis=0)
                label_predicted = np.append(label_predicted, logits.detach().cpu().numpy(), axis=0)
                all_input_ids = np.append(all_input_ids, inputs["input_ids"].detach().cpu().numpy(), axis=0)
                all_phrase_spans = np.append(all_phrase_spans, inputs["phrase_spans"].detach().cpu().numpy(), axis=0)
                all_num_phrases = np.append(all_num_phrases, inputs["num_phrases"].detach().cpu().numpy(), axis=0)
    label_predicted = np.argmax(label_predicted, axis=-1)
    outputs, results = compute_metrics(
        label_truth, label_predicted, all_input_ids, all_phrase_spans, all_num_phrases, tokenizer
    )
